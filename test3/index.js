...
const ProtobufType = protobuf.load(path.join(path.dirname(__filename), 'test3.proto'))
    .then(root => root.lookupType('testpackage.IntIdsResource'));
...
Promise.all(Serializers)
    .then(serializers => {
        serializers.forEach(([ serializerName, encodeFn, decodeFn ]) => {
            ...
            const buf = encodeFn({ ids: [ 1023001, 1024621, 32775, 78109 ] });
